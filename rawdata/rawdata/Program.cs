...
namespace ConsoleApp1
{
    class Program
    {
        ...
        public static StreamWriter eegdata = new StreamWriter(@"Swapnil_blue_EEGData_" + string.Format("{0:yyyy-MM-dd_hh-mm-ss-fff}", DateTime.Now) + ".csv", true);
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                MqttClient clientmq = new MqttClient("192.168.43.245");
                        ...
                        foreach (string s in packets)
                        {
                            try
                            {
                                ...
                                if (ready)
                                {
                                    ...
                                    if (startRead)
                                    {                                                                            
                                        MqttClient clientmq = new MqttClient("192.168.43.131");
                                        clientmq.ProtocolVersion = MqttProtocolVersion.Version_3_1;
                                        byte code = clientmq.Connect(Guid.NewGuid().ToString(), "username", "qwertyuiop");
